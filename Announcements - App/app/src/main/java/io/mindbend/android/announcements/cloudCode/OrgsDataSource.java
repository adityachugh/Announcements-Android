...
//                int i = 0;
    ...
    public static void getRequestedPendingPrivateOrganizationUsers (final View view, final Context context, final ProgressBar loading,
        ...
        if (!App.hasNetworkConnection(context)){
            ...
            ParseCloud.callFunctionInBackground("getRequestedPendingPrivateOrganizationUsers", params, new FunctionCallback<List<ParseObject>>() {
                @Override
                public void done(List<ParseObject> parseObjects, ParseException e) {
                    loading.setVisibility(View.GONE);
                    layoutView.setVisibility(View.VISIBLE);
                    ...
                    if (e == null){
                        ArrayList<User> users = new ArrayList<User>();
                        HashMap<User, Integer> userTypes = new HashMap<User, Integer>(); //will be all pending, but needed for displaying list
